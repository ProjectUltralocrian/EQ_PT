...
void NewProjectAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //m_LeftChain.get<HPF>().coefficients = juce::dsp::IIR::Coefficients<float>::makeFirstOrderHighPass(   (getSampleRate(), m_TreeState.getRawParameterValue(ParameterNames[params::HPF_FREQ])->load());
    ...
    m_LeftChain.setBypassed<LMF>(m_TreeState.getRawParameterValue(ParameterNames[params::LOW_MID_BYPASS])->load() ? true : false);
    m_RightChain.setBypassed<LMF>(m_TreeState.getRawParameterValue(ParameterNames[params::LOW_MID_BYPASS])->load() ? true : false);
...
{   
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>(ParameterNames[params::LOW_MID_FREQ], ParameterNames[params::LOW_MID_FREQ], juce::NormalisableRange<float>(20.f, 20000.f, 1.f, 0.25f), 100.f));
